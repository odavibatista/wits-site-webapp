...
import { createActivity } from '@/app/api/activities/create-activity.endpoint'
...
export async function create(
  ...
  const parsed = ActivitySchema.create.safeParse({
  ...
  try {
    ...
    const res = await createActivity(token, data)

    if (isApiError(res)) {
      return {
        errors: { _apiResponse: res.message },
      }
    }
  } catch (err) {
    return {
